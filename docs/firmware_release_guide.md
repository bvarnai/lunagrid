...
Before compiling the firmware, you must update the version tag inside the source code:

1. **Open the C++ source file**:
   Locate `firmware/src/main.cpp` and find the version constant near the top:
   ```cpp
   const char* FIRMWARE_VERSION = "1.1.0"; // Increment this version (e.g. 1.0.0 -> 1.1.0)
   ```
   *Note: The target version string in the database must exactly match this string to satisfy verification checks.*

2. **Compile the binary**:
   Run the PlatformIO compiler from the `firmware/` directory:
   ```bash
   pio run
   ```
   
3. **Locate the compiled binary**:
   Once compile completes, the compiled binary is located at:
